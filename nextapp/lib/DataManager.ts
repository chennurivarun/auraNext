...
import { User, Dealer, Vehicle, Transaction, Notification, Feedback, RTOApplication, SystemConfig, MarketingAsset } from '@/lib/entities';
import { SecurityValidator } from './SecurityValidator';
import { Core } from '@/lib/integrations';
...
export class DataManager {
  ...
  static async processAIFeedback(feedbackData) {
    try {
      ...
      const insights = await Core.InvokeLLM({
  ...
  static async getPersonalizedMarketingRecommendations(dealerId) {
    try {
      ...
      const recommendations = await Core.InvokeLLM({
