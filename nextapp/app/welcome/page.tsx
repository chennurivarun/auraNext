...
export default function Welcome() {
  ...
  const handleAuth = async () => {
    await User.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: window.location.origin + createPageUrl('SplashScreen') }
    });
  };
      ...
      <div className="relative z-10 w-full max-w-md text-center">
        <div className="bg-white/10 backdrop-blur-lg p-8 md:p-12 rounded-2xl shadow-2xl border border-white/20">
          ...
          <p className="text-lg text-gray-200 mb-10">
            Welcome to Aura. Your new competitive edge.
          </p>
          <div className="space-y-4">
            <Button onClick={handleAuth} className="w-full h-14 text-lg momentum-btn-accent flex items-center justify-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48" className="fill-current">
